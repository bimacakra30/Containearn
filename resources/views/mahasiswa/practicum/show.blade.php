                ...
                <section class="grid min-h-0 flex-1 gap-4 xl:grid-cols-[450px,minmax(0,1fr),400px]">
                    ...
                    <section class="glass flex min-h-0 flex-col rounded-[1.75rem] p-6">
                        ...
                        <div class="mt-4 min-h-0 flex-1">
                            <div class="h-full rounded-[1.5rem] border border-slate-200 bg-slate-950 p-4 text-sm text-slate-100">
                                <pre class="h-full w-full overflow-auto whitespace-pre-wrap font-mono">{{ $currentAnswer['stdout'] ?? 'No output yet.' }}</pre>
                            </div>
                        </div>
                    </section>
                </section>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const timerStorageKey = `practicum_timer_{{ $module->id_module }}`;
    ...
    if (timerElement && endSessionForm) {
        ...
        let expiresAt = useStoredDeadline
            ? Number(storedState.expiresAt)
            : serverExpiresAt;
        let hasEnded = false;
    ...
    if (!container || !input || !form || typeof window.require === 'undefined') {
        return;
    }
